...
* The advantage of minimizing the KL of $q_\phi(\mathbf{z}|\mathbf{x})$ with $\mathcal{N}(0, 1)$ is that the KL takes a **closed form** when the distributions are normal, i.e. there is no need to compute the expectation over all possible latent representations $\mathbf{z}$, nor to take samples:
...
$$\min_\theta \, \text{KL}(p(x) || p_\theta(x)) = \mathbb{E}_{x \sim p(x)} [- \log \dfrac{p_\theta(x)}{p(x)}]$$
...
* In supervised learning, we are learning the **conditional probability** $p(t | x)$ of the targets given the inputs.
...
$$\text{KL}(p(t | x) || p_\theta(t | x)) = \mathbb{E}_{x, t \sim p(t | x)} [- \log \dfrac{p_\theta(t | x)}{p(t | x)}]$$
...
$$\begin{aligned}
\text{KL}(p(t | x) || p_\theta(t | x)) &= \mathbb{E}_{x, t \sim p(t | x)} [- \log p_\theta(t | x)]  - \mathbb{E}_{x, t \sim p(t, x)} [- \log p(t | x)] \\
&\\
...
\nabla_\theta \, \text{KL}(p(t | x) || p_\theta(t | x))  & = \nabla_\theta \, H(p(t | x), p_\theta(t |x)) - \nabla_\theta \,  H(p(t|x)) \\
    &\\
     & = \nabla_\theta \, H(p(t | x), p_\theta(t | x)) \\
     & \\
     & = \nabla_\theta \, \mathbb{E}_{x, t \sim \mathcal{D}} [-  \log p_\theta(t | x) ]\\
\end{aligned}
$$
...
$$
\mathbb{E}_{x \sim p(x)} [\log p_\theta(x)] \approx \dfrac{1}{N} \, \sum_{i=1}^N \log p_\theta(x_i) = \dfrac{1}{N} \, \log \prod_{i=1}^N p_\theta(x_i) = \dfrac{1}{N} \, \log L(\theta)
$$
...
* If we had a **generative model** telling us how a point on the manifold $z$ maps to the image space ($p(x | z)$), we would only need to learn the distribution of the data in the lower-dimensional **latent space**.
...
* If we knew the distribution $p_\theta(z)$ of the manifold (latent space), we could infer the distribution of the data $p_\theta(x)$ using that model throug **marginalization**:
...
* **Variational inference** proposes to approximate the **true posterior** (encoder) $p_\theta(z | x)$ by another parameterized posterior distribution $q_\phi(z|x)$ (encoder). 

![Source: <https://lilianweng.github.io/lil-log/2018/08/12/from-autoencoder-to-beta-vae.html>](img/VAE-graphical-model.png){width=55%}
