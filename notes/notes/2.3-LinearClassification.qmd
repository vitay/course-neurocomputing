...
![Binary linear classification of 2D data.](../slides/img/classification-animation1.png)
...
![The hyperplane separates the input space into two regions.](../slides/img/classification-animation2.png)
...
![Projection on an hyperplane.](../slides/img/projection.svg)
...
![Batch perceptron algorithm.](../slides/img/classification-animation.gif)
...
![Online perceptron algorithm.](../slides/img/classification-animation-online.gif)
...
![Normal distributions with different parameters $\mu$ and $\sigma$ explain the data with different likelihoods.](../slides/img/MLE2.png)
...
![Logistic regression for soft classification. The confidence scores tells how certain the classification is.](../slides/img/logisticregression-animation.gif)
...
![Example of **One-vs-All** classification: one binary classifier per class. Source <http://cs231n.github.io/linear-classify>](../slides/img/pixelspace.jpg)
...
![Linear perceptron for images. The output is the logit score. Source <http://cs231n.github.io/linear-classify>](../slides/img/imagemap.jpg)
...
![The logit scores $\mathbf{z}$ cannot be compared to the targets $\mathbf{t}$: we need to transform them into a probability distribution $\mathbf{y}$.](../slides/img/softmax-transformation.png)
...
![Softmax activation transforms the logit score into a probability distribution. Source <http://cs231n.github.io/linear-classify>](../slides/img/softmax-comp.png){width=100%}
...
![Cross-entropy between two distributions $X$ and $Y$: are samples of $X$ likely under $Y$?](../slides/img/crossentropy.svg){width=100%}
...
![Minimizing the cross-entropy between $\mathbf{t} | \mathbf{x}$ and $\mathbf{y} | \mathbf{x}$ makes them similar.](../slides/img/crossentropy-animation.gif){width=100%}
...
![](../slides/img/softmaxclassifier.svg){width=70%}
...
The **training error** is the error made on the training set. It is easy to measure for classification: number of misclassified examples divided by the total number.

$$
    \epsilon_\mathcal{D} = \dfrac{\text{number of misclassifications}}{\text{number of examples}}
$$

It is totally irrelevant on usage: reading the training set has a training error of 0%.
What matters is the **generalization error**, which is the error that will be made on new examples (not used during learning). The generalization error is much harder to measure (potentially infinite number of new examples, what is the correct answer?) and often approximated by the **empirical error on the test set**: one keeps a number of training examples out of the learning phase and one tests the performance on them.
We need **cross-validation** to detect **overfitting**.
...
![Source: <https://alliance.seas.upenn.edu/~cis520/dynamic/2017/wiki/index.php?n=Lectures.PrecisionRecall>](../slides/img/TP_FP.png)
...
![](../slides/img/confusionmatrix.png)
