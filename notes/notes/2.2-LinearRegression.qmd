...
$$
    \mathcal{L}(w, b) = \dfrac{1}{N} \, \sum_{i=1}^{N} (t_i - y_i )^2
$$
...
$$
\begin{cases}
    \Delta w = - \eta \, \dfrac{\partial \mathcal{L}(w, b)}{\partial w}\\
    \\
    \Delta b = - \eta \, \dfrac{\partial \mathcal{L}(w, b)}{\partial b}\\
\end{cases}
$$
...
$$
    \dfrac{\partial \mathcal{L}(w, b)}{\partial w} = \dfrac{\partial}{\partial w} [\dfrac{1}{N} \, \sum_{i=1}^{N} (t_i - y_i )^2]
$$

Partial derivatives are linear, so the derivative of a sum is the sum of the derivatives:

$$
    \dfrac{\partial \mathcal{L}(w, b)}{\partial w} = \dfrac{1}{N} \, \sum_{i=1}^{N} \dfrac{\partial}{\partial w} (t_i - y_i )^2
$$
...
$$
    \dfrac{\partial \mathcal{L}(w, b)}{\partial w} = \dfrac{1}{N} \, \sum_{i=1}^{N} \dfrac{\partial}{\partial w} \mathcal{l}_i(w, b)
...
$$
    \dfrac{ d f(g(x))}{dx} = \dfrac{ d f(g(x))}{d g(x)} \times \dfrac{ d g(x)}{dx} = \dfrac{ d f(y)}{dy} \times \dfrac{ d g(x)}{dx}
$$

The first derivative considers $g(x)$ to be a single variable. Applied to our problem, this gives:

$$
     \dfrac{\partial}{\partial w} \mathcal{l}_i(w, b) =  \dfrac{\partial g_i(y_i)}{\partial y_i} \times  \dfrac{\partial y_i}{\partial w}
$$

The square error function $g_i(y) = (t_i - y)^2$ is easy to differentiate w.r.t $y$:

$$
    \dfrac{\partial g_i(y_i)}{\partial y_i} = - 2 \, (t_i - y_i)
$$
...
$$
\begin{cases}
   \dfrac{\partial  y_i}{\partial w} = x_i \\
    \\
   \dfrac{\partial  y_i}{\partial b} = 1 \\
\end{cases}
$$
...
$$
\begin{cases}
    \dfrac{\partial \mathcal{l}_i(w, b)}{\partial w} = - 2 \, (t_i - y_i) \, x_i \\
    \\
    \dfrac{\partial \mathcal{l}_i(w, b)}{\partial b} = - 2 \, (t_i - y_i) \\
\end{cases}
$$
...
$$
\begin{cases}
    \dfrac{\partial \mathcal{L}(w, b)}{\partial w} = - \dfrac{2}{N} \sum_{i=1}^{N} (t_i - y_i) \, x_i \\
    \\
    \dfrac{\partial \mathcal{L}(w, b)}{\partial b} = - \dfrac{2}{N} \sum_{i=1}^{N} (t_i - y_i) \\
\end{cases}
$$
...
$$
\begin{cases}
    \Delta w = \eta \, \dfrac{1}{N} \sum_{i=1}^{N} (t_i - y_i) \, x_i \\
    \\
    \Delta b = \eta \, \dfrac{1}{N} \sum_{i=1}^{N} (t_i - y_i) \\
\end{cases}
$$
...
* **for** M epochs:
    ...
    * $\Delta w = \eta \, \dfrac{1}{N} dw$

    * $\Delta b = \eta \, \dfrac{1}{N} db$
:::
...
$$
    \min_{W, \mathbf{b}} \, \mathcal{L}(W, \mathbf{b}) = \mathbb{E}_\mathcal{D} [ ||\mathbf{t} - \mathbf{y}||^2 ] \approx \dfrac{1}{N} \, \sum_{i=1}^N ||\mathbf{t}_i - \mathbf{y}_i||^2 = \dfrac{1}{N} \, \sum_{i=1}^N \mathcal{l}_i(W, \mathbf{b})
$$
...
$$
    y = \sigma(w \, x + b )  = \displaystyle\dfrac{1}{1+\exp(-w \, x - b )}
$$

![Logistic or sigmoid function $\sigma(x)=\displaystyle\dfrac{1}{1+\exp(-x)}$.](../slides/img/sigmoid.png){width=60%}
...
$$
\begin{aligned}
    \displaystyle\dfrac{\partial l_i(w, b)}{\partial w}
        &= 2 \, (t_i - y_i)  \, \dfrac{\partial}{\partial w}  (t_i - \sigma(w \, x_i + b ))\\
        &\\
...
The logistic function $\sigma(x)=\dfrac{1}{1+\exp(-x)}$ has the nice property that its derivative can be expressed easily:
...
:::{.callout-note}
Here is the proof using the fact that the derivative of $\displaystyle\dfrac{1}{f(x)}$ is $\displaystyle\dfrac{- f'(x)}{f^2(x)}$ :

$$\begin{aligned}
    \sigma'(x) & = \displaystyle\dfrac{-1}{(1+\exp(-x))^2} \, (- \exp(-x)) \\
    &\\
    &= \dfrac{1}{1+\exp(-x)} \times \dfrac{\exp(-x)}{1+\exp(-x)}\\
    &\\
    &= \dfrac{1}{1+\exp(-x)} \times \dfrac{1 + \exp(-x) - 1}{1+\exp(-x)}\\
    &\\
    &= \dfrac{1}{1+\exp(-x)} \times (1 - \dfrac{1}{1+\exp(-x)})\\
    &\\
    &= \sigma(x) \, (1 - \sigma(x) )\\
\end{aligned}
$$
...
$$
    \epsilon_\mathcal{D} = \dfrac{1}{N} \, \sum_{i=1}^{N} (t_i - y_i )^2
$$
...
$$
    R^2 = 1 - \dfrac{\text{Var}(\text{residuals})}{\text{Var}(\text{data})} = 1 - \dfrac{\sum_{i=1}^N (t_i- y_i)^2}{\sum_{i=1}^N (t_i - \hat{t})^2}
$$
...
* Randomly split the data $\mathcal{D}$ into $k$ subsets of $\dfrac{N}{k}$ examples $\{ \mathcal{S}_{1}, \dots , \mathcal{S}_{k}\}$
...
* For all models $M_i$:
    ...
    * The empirical error of the model $M_i$ on $\mathcal{D}$ is the average of empirical errors made on $(\mathcal{S}_j)_{j=1}^{k}$
        ...
        $$
            \epsilon_{\mathcal{D}} (M_i) = \dfrac{1}{k} \cdot \sum_{j=1}^{k} \epsilon_{\mathcal{S}_j}(h_{ij})
        $$
...
The bare minimum in ML is to have separate training and test sets. However, the test set should only be used once. If you try many variations of the same algorithm on a single test set and keep the best one, you end up overfitting the test set: the model may not generalize well to novel data...

A third **validation set** is typically used to track overfitting during training and perform model selection. The test set is ultimately used to report the final performance.

![Source: <https://developers.google.com/machine-learning/crash-course/validation/another-partition>](../slides/img/validationset.svg)
